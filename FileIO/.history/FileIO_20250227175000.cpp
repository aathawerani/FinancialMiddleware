...
long CFileIO::WriteLine(const std::vector<unsigned char>& buffer) {
    std::lock_guard<std::mutex> lock(fileMutex);
    if (!fileStream.is_open()) {
...
std::string CFileIO::ReadBinary() {
    ...
    fileStream.seekg(0, std::ios::beg);  // Move back to start

    if (fileSize <= 0) {
        throw std::runtime_error("Error: File is empty or unreadable!");
    }

    std::string fileData(fileSize, '\0');  // Allocate memory
    fileStream.read(&fileData[0], fileSize);  // Read file content
