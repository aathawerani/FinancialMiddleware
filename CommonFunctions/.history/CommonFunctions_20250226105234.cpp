...
int CommonFunctions::ebcdicToAscii(unsigned char* buffer, int length) {
    ...
    for (int i = 0; i < length; ++i) {
        buffer[i] = EbcdicToAsciiTable[buffer[i]];
    }
    return 1;
}

int CommonFunctions::asciiToEbcdic(unsigned char* buffer, int length) {
    ...
    for (int i = 0; i < length; ++i) {
        buffer[i] = AsciiToEbcdicTable[buffer[i]];
    }
    return 1;
}