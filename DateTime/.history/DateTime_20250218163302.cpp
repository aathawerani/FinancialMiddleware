...
{
	if (GetDateFormatW (0, 0, &systemTime, L"dddd", (LPWSTR)cBuffer, DT_STRING_LEN) > 0)
		return true;
	else
		return false;
}
...
bool CDateTime::toFormattedString (std::string& buffer)
{
	...
	oss << std::setw(4) << std::setfill('0') << universalTime.wYear
		...
		<< std::setw(2) << std::setfill('0') << universalTime.wDay
		<< std::setw(2) << std::setfill('0') << universalTime.wHour
